...
#%% Predicting country on full dataset
...
df.drop(["variety", "id", "color"], inplace=True, axis = 1)
...
label_names = flatten([l.lower().split(" ") for l in list(set(df.country))])
...
# create documents
documents = list(zip(df.description, df.country))
...
# dictionary
d = {'country':sorted(labels_list), 'Probability':probs_list}
