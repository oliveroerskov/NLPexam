...
#%% Predicting color on full dataset
...
df.drop(["variety", "id", "country"], inplace=True, axis = 1)
...
label_names = flatten([l.lower().split(" ") for l in list(set(df.color))])
...
# create documents
documents = list(zip(df.description, df.color))
...
## Bernoulli NB
clf_color = BernoulliNB()
clf_color.fit(train_features, train_labels)
test_predictions = clf_color.predict(test_features)
...
# dictionary
d = {'color':sorted(labels_list), 'Probability':probs_list}
